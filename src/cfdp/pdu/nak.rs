...
pub fn get_max_segment_requests_for_max_packet_size_and_pdu_config(
...
impl<'buf> NakPduCreatorWithReservedSeqReqsBuf<'buf> {
    pub fn new(buf: &'buf mut [u8], mut pdu_header: PduHeader, num_segment_reqs: usize) -> Self {
...
impl NakPduCreatorWithReservedSeqReqsBuf<'_> {
    ...
    pub fn finish(self) -> Result<usize, ByteConversionError> {
        let expected_len = self.len_written();
        ...
        let mut current_idx = self.pdu_header.write_to_bytes(self.buf)?;
...
mod tests {
    ...
    fn test_with_reserved_lost_segment_buf_no_segments_normal_file_0() {
        ...
        let mut buf: [u8; 64] = [0; 64];
        let pdu_header = PduHeader::new_no_file_data(pdu_conf, 0);
        let mut nak_pdu = NakPduCreatorWithReservedSeqReqsBuf::new(&mut buf, pdu_header, 0);
        ...
        let len_written = nak_pdu.finish().unwrap();
    ...
    fn test_with_reserved_lost_segment_buf_invalid_scope() {
        ...
        let mut buf: [u8; 64] = [0; 64];
        let pdu_header = PduHeader::new_no_file_data(pdu_conf, 0);
        let mut nak_pdu = NakPduCreatorWithReservedSeqReqsBuf::new(&mut buf, pdu_header, 2);
        assert_eq!(
            nak_pdu
    ...
    fn test_with_reserved_lost_segments_buf_two_segments_normal_file() {
        ...
        let pdu_header = PduHeader::new_no_file_data(pdu_conf, 0);
        let mut nak_pdu =
            NakPduCreatorWithReservedSeqReqsBuf::new(&mut buf, pdu_header, num_segments);
        nak_pdu
            .set_start_and_end_of_scope(100, 200)
            .expect("setting start and end of scope failed");
